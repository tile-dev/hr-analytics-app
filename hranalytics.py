...
def preprocess_data(data):
    ...
    data = data.drop(
        columns=["Employee_Name", "EmpID", "DOB", "DateofHire", "DateofTermination", "LastPerformanceReview_Date"]
    )
...
def predict_employee_attrition(data, top_n_percent=10):
    ...
    at_risk_employees = pd.DataFrame(
        {"Employee_Name": employee_names.loc[still_employed_index], "Attrition_Probability": attrition_probabilities}
    )
...
def key_drivers_of_attrition(data, top_n=20):
    ...
    feature_importance_df = feature_importance_df[
        ~feature_importance_df["Features"].str.startswith(tuple(exclude_features))
    ]
...
def plot_salary_distribution(data):
    chart = (
        alt.Chart(data)
        .mark_bar()
        .encode(alt.X("Salary:Q", bin=alt.Bin(maxbins=30), title="Salary"), y="count()")
...
def plot_employees_by_dept(data):
    chart = (
        alt.Chart(data)
        .mark_bar()
        .encode(x="Department:N", y="count()", color="Department:N")
...
def plot_gender_distribution(data):
    chart = (
        alt.Chart(data).mark_bar().encode(x="Sex:N", y="count()", color="Sex:N").properties(title="Gender Distribution")
    )
    return chart
...
def plot_performance_by_department(data):
    chart = (
        alt.Chart(data)
        .mark_bar()
        .encode(x="Department:N", y="average(PerfScoreID):Q", color="Department:N")
...
def plot_employee_satisfaction(data):
    chart = (
        alt.Chart(data)
        .mark_bar()
        .encode(x="EmpSatisfaction:N", y="count()", color="EmpSatisfaction:N")
