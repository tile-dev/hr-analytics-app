...
def breakdown(col1, col2):
    ...
    chart = (
        ...
        .encode(
            ...
            tooltip=[
                alt.Tooltip(f"{col1}:N", title=col1),
                alt.Tooltip(f"{col2}:N", title=col2),
                alt.Tooltip("Count:Q", title="Count"),
            ],
        )
        .properties(title=f"Breakdown of {col1} by {col2}", width=600, height=400)
    )
...
v = dp.View(
    ...
    dp.Page(
        dp.Form(
            controls=[dp.Choice(options=list(df["Employee_Name"].unique()), name="employee_name")],
